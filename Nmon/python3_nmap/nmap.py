...
class PortScannerAsync(object):
    ...
    def scan(self, hosts='127.0.0.1', ports=None, arguments='-sV', callback=None, queue=None):
        """
        ...
        def scan_progressive(self, hosts, ports, arguments, callback,queue):
            for host in self._nm.listscan(hosts):
                ...
                if callback is not None and isinstance(callback, collections.Callable):
                    callback(host, scan_data,queue,self._nm.get_nmap_last_output())
            return

        self._process = Process(
            target=scan_progressive,
            args=(self, hosts, ports, arguments, callback,queue)
            )
